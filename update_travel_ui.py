...
new_ui = """import React, { useState } from 'react';
...
import { Plane, Plus, FileText, CheckCircle2, IndianRupee, Receipt } from 'lucide-react';
      ...
      <Modal isOpen={settleModalOpen} onClose={() => setSettleModalOpen(false)} title="Verify & Settle Claim">
        <div className="space-y-4">
          ...
          <div className="bg-gray-50 dark:bg-gray-800 p-4 rounded-lg space-y-3">
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div><span className="text-gray-500">Employee:</span> <span className="font-medium text-navy-900 dark:text-white">{selectedRequest?.employee?.firstName} {selectedRequest?.employee?.lastName}</span></div>
              <div><span className="text-gray-500">Bills Link:</span> <a href={selectedRequest?.billUpload} target="_blank" className="text-indigo-600 hover:underline break-all">{selectedRequest?.billUpload}</a></div>
            </div>
