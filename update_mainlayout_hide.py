    ...
    <div className="flex h-screen bg-[#f4f7f6] dark:bg-[#0b1120] transition-colors overflow-hidden relative">
      ...
      <div 
        className={`fixed inset-y-0 left-0 z-50 transform transition-all duration-300 ease-in-out lg:relative 
          ${sidebarOpen ? 'translate-x-0 lg:w-[17.5rem] lg:opacity-100' : '-translate-x-full lg:w-0 lg:opacity-0 lg:-ml-4 overflow-hidden'}`}
      >
...
with open("client/src/components/layout/MainLayout.tsx", "w", encoding="utf-8") as f:
    f.write(code)
print("Updated MainLayout.tsx for full hide/show")
